...
for (s_id in which(samplesheet[,1] %in% files)) {
  ...
  result_file = paste0("allelic-imbalances/res/no-segmentation/", TCGA_id, ".png")
  ...
  result_file = paste0("allelic-imbalances/res/auto-segmentation/", TCGA_id, ".png")
  png(result_file, res=600, width=7000, height=2850)  
  {
    par(mar=c(5,7,5,3))
    ylim = c(-1.5,1.2)
  ...
  for (arm in unique(baf$ARM)) {
    ss = segmentation(baf$BAF[which(baf$ARM == arm)], n=n, g_size=g_size)
